...
export function SidebarNav(props: Props) {
  return (
    <nav className="nav-stack" aria-label="main">
      ...
      <button type="button" className={props.extensionsOpen ? 'nav-item on' : 'nav-item'} title={t('navPluginsHint')} aria-label={t('navPlugins')} onClick={props.onOpenExtensions}>
        <span className="nav-ico"><IconPlugins /></span><span className="nav-label">{t('navPlugins')}</span>
      </button>
      <button type="button" className={props.scheduledOpen ? 'nav-item on' : 'nav-item'} title={t('navScheduledHint')} aria-label={t('navScheduled')} onClick={props.onOpenScheduled}>
