...
pub async fn git_file_diff(cwd: String, path: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        // Git's textual diff is generated by Git and does not dereference a
        // working-tree symlink for this UI. The fallback previews below do,
        // however, so apply the same containment proof used by non-Git
        // workspaces before reading or enumerating a local target.
        let preview_full = if full.exists() {
            existing_project_child(root, &rel)?
        } else {
            full.clone()
        };
        ...
        // new / untracked file
        if preview_full.is_file() {
            match std::fs::read_to_string(&preview_full) {
                Ok(content) => {
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn git_preview_rejects_symlink_outside_project() {
        use std::os::unix::fs::symlink;
        use std::process::Command;
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let base = std::env::temp_dir().join(format!("gorkx-git-preview-{nonce}"));
        let project = base.join("project");
        let outside = base.join("outside.txt");
        fs::create_dir_all(&project).unwrap();
        fs::write(&outside, "private").unwrap();
        ...
        assert!(matches!(result, Err(message) if message.contains("outside the project")));
        fs::remove_dir_all(base).unwrap();
    }
}
