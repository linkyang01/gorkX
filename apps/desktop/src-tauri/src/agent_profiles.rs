...
fn agents_dir() -> PathBuf {
    crate::paths::grok_home().join("agents")
}
        ...
        .filter(|value| !value.is_empty())
    {
        ...
        None => {
            ...
            if is_safe_name(&name) {
                ...
                Err("Role name is not supported.".into())
            }
        }
    }
}
...
fn refuse_symlink(path: &Path) -> Result<(), String> {
    if let Ok(meta) = std::fs::symlink_metadata(path) {
        if meta.file_type().is_symlink() {
            return Err("Agent profile path must not be a symbolic link.".into());
        }
    }
    Ok(())
}
...
fn read_profile(path: &Path, source: &str, scope: &str) -> Result<AgentProfileSummary, String> {
    ...
    if content.len() > 64 * 1024 {
        return Err("Agent profile is too large.".into());
    }
    let trimmed = content.trim_start();
    if !trimmed.starts_with("---") {
        return Err("Agent profile is missing YAML frontmatter.".into());
    }
    let Some(end) = trimmed[3..].find("\n---") else {
        return Err("Agent profile frontmatter is incomplete.".into());
    };
    ...
    for line in yaml.lines() {
        ...
        if let Some(value) = line.trim().strip_prefix("displayName:") {
    ...
    if !is_safe_name(&name) {
        return Err("Agent profile has an invalid name.".into());
    }
    if description.is_empty() {
        description = "Custom Grok Build role".into();
    }
    if display_name.is_empty() {
        display_name = name.clone();
    }
    let editable = name.starts_with(PREFIX)
        && path.file_name().and_then(|v| v.to_str()) == Some(&format!("{name}.md"));
...
pub fn agent_profiles_list(project: Option<String>) -> Result<Vec<AgentProfileSummary>, String> {
    ...
    let mut read_dir = |dir: PathBuf, source: &str, scope: &str| -> Result<(), String> {
        ...
        for entry in std::fs::read_dir(&dir).map_err(|e| format!("read agent profiles: {e}"))? {
            ...
            if let Ok(profile) = read_profile(&path, source, scope) {
...
) -> Result<AgentProfileSummary, String> {
    let display_name = display_name.trim().replace(['\r', '\n'], " ");
    if display_name.is_empty() || display_name.chars().count() > 80 {
        return Err("Role name must be between 1 and 80 characters.".into());
    }
    let description = description.trim().replace('\n', " ");
    if description.is_empty() || description.chars().count() > 240 {
        return Err("Role description must be between 1 and 240 characters.".into());
    }
    let instructions = instructions.replace("\r\n", "\n").trim().to_string();
    if instructions.is_empty()
        || instructions.chars().count() > MAX_BODY_CHARS
        || instructions.contains('\0')
    {
        return Err("Role instructions must be between 1 and 12000 valid characters.".into());
    }
    let dir = agents_dir();
...
pub fn agent_profile_remove(name: String) -> Result<(), String> {
    let name = name.trim();
    if !name.starts_with(PREFIX) || !is_safe_name(name) {
        return Err("Only gorkX-created roles can be removed here.".into());
    }
    let path = agents_dir().join(format!("{name}.md"));
    refuse_symlink(&path)?;
    if path.exists() {
        std::fs::remove_file(path).map_err(|e| format!("remove agent profile: {e}"))?;
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn accepts_engine_safe_role_names() {
        assert!(is_safe_name("gorkx-report_writer"));
        assert!(!is_safe_name("gorkx role"));
    }
    #[test]
    fn edit_keeps_the_existing_native_identity() {
        assert_eq!(
            resolve_role_name("改名后的角色", Some("gorkx-report-writer")).unwrap(),
            "gorkx-report-writer"
        );
        assert!(resolve_role_name("anything", Some("outside-role")).is_err());
    }
    #[test]
    fn non_ascii_display_name_still_gets_a_safe_engine_name() {
