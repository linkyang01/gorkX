...
**当前公开基线（2026-08-31）**：gorkX `1.3.1`；包内内核 `grok 1.0.12 (bc7f02eddd3d)`，含补丁 0001–0007。**本分支本地候选（2026-09-01）**：gorkX `1.3.2`，新增并已核验补丁 0008；没有创建 tag、DMG 或 GitHub Release。
**发布边界**：本分支不宣称 Developer ID/公证、干净机安装重开或原生 Intel 验收；这些范围仍由发布门禁单独记录。

**2026-08-09 GUI 复核结果**：B1 真实首轮、B3 委派启动→完成结果、账户/额度、模型列表、外观深色、`+` 桌面动作、退出重开恢复已通过；麦克风、干净机和第三方 endpoint 仍按下方门槛保留为未验收。会话级快捷操作位于输入框上方独立条带，详见 [`VALIDATION_EVIDENCE.md`](VALIDATION_EVIDENCE.md)。
...
| B2 | Composer 上方快捷操作条 **委派子任务** 或 ⇧⌘B | 引导面板；默认只读 | |
