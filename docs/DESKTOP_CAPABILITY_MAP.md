...
| `--max-turns` | Settings: General → Task run limit | An optional numeric 1–200 limit is passed to each new or reconnected Agent process. Leaving it blank preserves the native default; existing running turns are never represented as retroactively limited. |
