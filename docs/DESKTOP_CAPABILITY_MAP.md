...
| `mcp add` (remote HTTPS), auth, setup, enable/disable and tool enable/disable | Extensions: MCP | A native name + HTTPS URL + HTTP/SSE form adds remote connections, explicitly scoped to all projects or the selected project; every live control is scoped to the active ACP task and OAuth stays in the provider browser flow. URL credentials, headers and arbitrary local commands remain advanced configuration rather than everyday fields. |
