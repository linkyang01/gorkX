...
| Conversation results | Markdown headings, lists, code, blockquotes, **step cards**, read-only completion checklists, **GFM pipe tables**, automatic charts for compact two-column numeric comparisons, safe data-only `chart` blocks, and common `mermaid` flowcharts | **Real** — visual results render inside the conversation automatically; tables retain their exact values. Chart blocks accept only bounded numeric bar/line data, while Mermaid support is a bounded local flowchart reader (TD/LR nodes and arrows), not a script runtime. No HTML, scripts or remote assets execute. |
