...
| + menu and `/` completion | `+` provides native forms and panels for normal work; `/` remains keyboard compatibility and expert completion | **Real** — task info, compact, export, skills, memory and extensions use desktop controls. When a live session advertises Grok Build's `deep-research` workflow, `+ → Deep research` opens a plain-language research brief form; it runs the kernel workflow and its progress stays in the conversation. Other live engine slash actions remain available through expert composer completion. |
