...
| + menu and `/` completion | `+` provides native forms and panels for normal work; `/` remains keyboard compatibility and expert completion | **Real** — task info, compact, export, skills, memory and extensions use desktop controls; live engine slash actions remain available only through the composer when the current session advertises them. |
