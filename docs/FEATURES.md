...
| Chat | Fork conversation | **Unavailable — hidden.** The 0.2.110 source contains a fork implementation, but its shipped `grok agent stdio` returns `Method not found` for `x.ai/session/fork`. gorkX therefore does not expose a button or slash shortcut until a locked kernel actually advertises the API. |
| Chat | Rewind conversation | **Unavailable — hidden.** The 0.2.110 `grok agent stdio` returns `Method not found` for `x.ai/rewind/points`; the desktop does not present its checkpoint dialog or execute path until the ACP extension becomes available. |
