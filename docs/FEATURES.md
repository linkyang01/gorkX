...
| Native decision interview | Grok Build `x.ai/ask_user_question` renders in the current task's message flow as a structured decision card with option context, multi-select, freeform answer, cancel, and plan-mode actions; a background task remains a modal safety fallback | **Kernel-wired** — every submit/cancel/plan action replies to the originating ACP request rather than sending guessed chat text. The response shape matches bundled Grok Build 0.2.112 source; it appears only when the engine actually asks a question, and a live engine turn is required to observe it |
