...
| GitHub | Review → Remote reads the current origin's open PRs; checks and discussion/review comments load on demand. A user-provided fine-grained read token stays in macOS Keychain. | **Read-only** — public origins are attempted anonymously first, but GitHub may rate-limit or block that path and gorkX then provides a Token-creation guide as an advanced fallback. gorkX never pushes a branch, creates a PR, or posts a remote comment. Official browser authorization and any remote write flow require their own complete authorization and confirmation design before they return to the product. |
