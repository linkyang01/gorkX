...
| Native subagents | Lifecycle events → persisted parent/child task tree; cancel and snapshots when advertised by the engine | **Half** — current locked kernel does not expose `x.ai/subagent/list_running`, so gorkX does not claim restart/reconnect recovery of running subagents; persisted historical rows become **unverified after reconnect**, never falsely running |
