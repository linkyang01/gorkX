...
| Native subagents | Lifecycle events → persisted parent/child task tree; completed children can be inspected and active children can be stopped directly from that tree; reconnect reconciles engine-reported running children; Settings policy for enabling all, `explore`, and `plan` types | **Kernel-wired** — 0.2.111 ACP exposes `list_running/get/cancel`; gorkX uses those runtime routes for the task tree. The current control plane does not provide a desktop creation/assignment route, so the kernel chooses delegation, capability mode and worktree isolation. |
