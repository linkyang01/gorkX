...
| Task deliverables | **本次成果 / Deliverables** button appears only when the active task has explicitly returned a file or media attachment; it opens the existing local preview / Finder flow | **Real, deliberately conservative** — it aggregates agent-returned attachments in the task and never scans a project directory or labels modified files as deliverables by guesswork. At present, bundled Grok Build ACP image output is the verified producer; other file types appear only when an agent actually returns them as attachments. |
