...
| Background workflows | Grok Build `workflow_updated` ACP events render as live cards in the originating conversation, with phases, active agents, budget consumption, elapsed time, pause reason and result summary; eligible cards offer Pause / Resume buttons | **Kernel-wired** — gorkX only projects bounded kernel updates and keeps a readable summary when a task is restored. Pause / Resume appears only when that live task advertises the kernel `workflow` command and its server-provided handle passes a strict safe-name check; the UI then dispatches the documented kernel workflow action. It does not create a second workflow scheduler or invent management controls for unavailable routes. |
