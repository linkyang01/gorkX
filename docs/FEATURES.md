...
| Settings · Browser | Connect / diagnose a version-pinned, isolated Playwright MCP for Chrome; optional origin allowlist; open an explicitly supplied HTTP(S) source in a separate gorkX reading window | **Real** — new connections use `@playwright/mcp@0.0.78`, an in-memory profile and blocked service workers. The user preview validates HTTP(S), has no gorkX credential bridge, and is separate from the Agent's Chrome MCP profile; browser tools still depend on Chrome and the kernel MCP runtime. |
