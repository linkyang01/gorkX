...
| Background workflows | Grok Build `workflow_updated` ACP events render as live cards in the originating conversation, with phases, active agents, budget consumption, elapsed time, pause reason and result summary | **Kernel-wired** — gorkX only projects bounded kernel updates and keeps a readable summary when a task is restored. It does not create a second workflow scheduler or pretend that TUI-only management routes are desktop controls. |
