...
| GitHub | Review → Remote reads the current origin's open PRs; checks and discussion/review comments load on demand. A user-provided fine-grained token stays in macOS Keychain. | **Real, confirmation-gated** — public origins are attempted anonymously first; a token is an advanced fallback for private repositories or rate limits. Review can create a PR only after the user supplies title/base/body, explicitly confirms, and has already pushed the branch; it never pushes a branch or edits local files. Posting a PR discussion comment is likewise a separate explicit confirmation. Official browser OAuth remains unshipped until gorkX has its own registered OAuth App configuration. |
