...
| Start screen | Button-first office and project workflows: read & summarize, draft a report, research & compare, plan a project, explore code, fix a bug, add a feature, write tests | **Real** — each card creates a normal durable Grok Build task with a localized, editable natural-language brief; it does not rely on a slash command or invent a separate agent loop. |
