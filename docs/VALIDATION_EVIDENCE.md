...
| 托管配置只读预览 | 包内 `grok setup --help`；`grok_admin` 固定白名单与前端环境面板 | 通过：0.2.112 的 `setup --json` 明确声明只打印获取的配置、不写入内核 home。gorkX 仅开放这条只读命令，拒绝无参数的安装命令。 | 未使用真实账户发起配置拉取；远端服务的可用性和返回内容仍需要登录后的人工阅读，不会自动安装。 |
