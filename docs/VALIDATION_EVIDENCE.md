...
| 前台 Computer 控制基础层 | Rust `computer` 单元测试（固定按键映射、文字边界/转义、坐标边界）；`computer_accessibility_status` / `computer_control_set_enabled` / `computer_control_emergency_stop`；设置页固定动作 | 通过：动作只经 `/usr/bin/osascript` 的固定脚本模板执行，启用前必须获得 macOS Accessibility 权限并确认；急停会禁用后续动作；不接受模型或任务传入的 AppleScript。 | 当前环境未授予 gorkX 辅助功能权限，因此未宣称一次真实点击/按键结果；仍需在已授权 Mac 上人工验收。Grok Build 0.2.116 的 `grok-computer` 仍是文件/终端工具集，没有本机 Computer ACP 自动调用协议。 |
