...
| 范围 | 命令 | 结果 | 边界 |
|---|---|---|---|
| TypeScript / 前端生产包 | `cd apps/desktop && npx tsc --noEmit && npm run verify:web-bundle` | 2026-07-23 通过；首屏 JS gzip 90,302 / 512,000 bytes | 不代表运行中的 Grok 真实对话 |
...
| 会话控制 ACP 探测 | `GORKX_ACP_TEST_HOME=/private/tmp/gorkx-acp-02110-home GORKX_ACP_TEST_CWD=/private/tmp/gorkx-worktree-acp-project node scripts/verify-grok-acp.mjs apps/desktop/src-tauri/resources/grok --authenticated --session-controls` | 2026-07-23 完成；`x.ai/session/fork` 与 `x.ai/rewind/points` 均明确返回 `Method not found` | 这证明当前 stdio 内核**未**暴露会话分叉/回退 API；gorkX 已隐藏相关入口，不把源码中存在但运行时不可达的实现算作功能 |
