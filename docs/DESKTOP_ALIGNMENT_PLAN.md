...
| 语音 | 原生按钮 | Composer | 🔶 | 1.3.2 本地候选已观察 PCM→STT→草稿；H3 干净短语/手动发送待完成 |
...
1. **委派子任务 UI**（部分已落地）：启动后立即写入 Process 树行、打开进程面板、有界轮询 `get` 更新状态；Inspect 展示 output/worktree（运行中也可）；Stop 立即标 cancelled；状态归一化 + 轮询清理 + 标签 i18n（`73ce3aa`+）。仍待人工点按验收与退出重开。  
2. **语音 H3**：TCC/in-process 采集与流式转写已修复并观察到草稿；仍需干净短语 → 草稿 → 手动发送。
3. **干净安装 H1 / 第三方模型 H2**（发布阻断）。  

### P1 — 深化 Real

1. Hooks 设置页失败提示与启停人工走查。  
   - **代码：** 成功/失败短文案、`settingsErrorMessage`、无任务空状态（`339127a`+）。  
   - **仍缺：** 真人点按信任/启停 + 失败 Hook 录屏。  
2. Computer：Accessibility 下截图/点击/急停一次。  
   - **代码：** 急停始终可点、启用态 danger、成功/失败 i18n（`339127a`）。  
   - **仍缺：** 已授权机上真实截图/点击/急停一次。  
3. GitHub：网页撤销后“验证连接”提示完整往返。  
   - **代码：** `githubFeedback` 成功/401/403、最后验证时间、重新认证 CTA（`3c35e86`）。  
   - **仍缺：** 用户在 GitHub 网页撤销后 live 点「验证连接」。  
