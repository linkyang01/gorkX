...
| Adjust running work | Composer → Adjust current work → native `x.ai/interject` request | 🟡 live kernel route decides whether the active turn can accept the adjustment. The desktop form never reports success until that request resolves; its error is retained on the task. |
